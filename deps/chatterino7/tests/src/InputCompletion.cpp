...
protected:
    ...
    template <typename T>
    auto queryTypedTabCompletion(const QString &fullQuery, bool isFirstWord)
    {
        EmoteSource source(this->channelPtr.get(), std::make_unique<T>());
        source.update(fullQuery);

        std::vector<StringCompletion> completions;
        source.addToStringCompletions(completions, 0, isFirstWord);
        return completions;
    }
...
TEST_F(InputCompletionTest,
       ThirdPartyGlobalScopeIsPlatformBoundWhileEmojiIsGlobal)
{
    const auto completion = queryClassicEmoteCompletion(":cat");
    const auto thirdParty =
        std::ranges::find_if(completion, [](const auto &item) {
            return item.displayName == QStringLiteral("CatBag") &&
                   item.providerName == QStringLiteral("Global FrankerFaceZ");
        });
    const auto emoji = std::ranges::find_if(completion, [](const auto &item) {
        return item.displayName == QStringLiteral("cat") && item.isEmoji;
    });

    ASSERT_NE(thirdParty, completion.end());
    ASSERT_NE(emoji, completion.end());
    ASSERT_EQ(thirdParty->availabilities.size(), 1U);
    ASSERT_EQ(emoji->availabilities.size(), 1U);
    EXPECT_EQ(thirdParty->availabilities[0].platform,
              QStringLiteral("twitch"));
    EXPECT_TRUE(emoji->availabilities[0].platform.isEmpty());

    const auto thirdPartyCandidate = draftCandidateFromEmoteItem(*thirdParty);
    const auto emojiCandidate = draftCandidateFromEmoteItem(*emoji);
    ASSERT_TRUE(thirdPartyCandidate.has_value());
    ASSERT_TRUE(emojiCandidate.has_value());
    EXPECT_EQ(thirdPartyCandidate->availability.platform,
              QStringLiteral("twitch"));
    EXPECT_TRUE(emojiCandidate->availability.platform.isEmpty());
}
...
TEST_F(InputCompletionTest,
       ClassicTypedTabCollisionPreservesProviderUnion)
{
    const auto completion =
        queryTypedTabCompletion<ClassicTabEmoteStrategy>("cla", false);
    const auto clap = std::ranges::find_if(completion, [](const auto &entry) {
        return entry.text == QStringLiteral("Clap ");
    });
    ASSERT_NE(clap, completion.end());
    EXPECT_TRUE(clap->emoteLike);
    ASSERT_TRUE(clap->emote.has_value());
    ASSERT_EQ(clap->emote->identityAlternatives.size(), 1U);
    const auto primaryProvider = clap->emote->identity.provider;
    const auto alternativeProvider =
        clap->emote->identityAlternatives[0].identity.provider;
    EXPECT_NE(primaryProvider, alternativeProvider);
    EXPECT_TRUE(primaryProvider == QStringLiteral("bttv") ||
                alternativeProvider == QStringLiteral("bttv"));
    EXPECT_TRUE(primaryProvider == QStringLiteral("7tv") ||
                alternativeProvider == QStringLiteral("7tv"));

    const auto clap2 = std::ranges::find_if(completion, [](const auto &entry) {
        return entry.text == QStringLiteral("Clap2 ");
    });
    ASSERT_NE(clap2, completion.end());
    ASSERT_TRUE(clap2->emote.has_value());
    EXPECT_EQ(clap2->emote->identity.provider, QStringLiteral("7tv"));
}
...
TEST(MultiChannelEmoteCompletion, DeduplicatesIdentityAndKeepsAmbiguousNames)
{
    const auto item = [](QString name, QString provider, QString id,
                         size_t child, QString context) {
        const auto emoteName = EmoteName{name};
        return EmoteItem{
            .emote = namedEmote(emoteName),
            .searchName = name,
            ...
            .availabilities = {
                DraftEmoteAvailability{
                    .platform = child == 0 ? QStringLiteral("twitch")
                                           : QStringLiteral("kick"),
                    .channelID = QString::number(child),
                },
            },
            .availableInChildren = {child},
            .availabilityContexts = {std::move(context)},
        };
    };

    std::vector items{
        item(QStringLiteral("Shared"), QStringLiteral("7tv"),
             QStringLiteral("shared-id"), 0, QStringLiteral("twitch/a")),
        item(QStringLiteral("Shared"), QStringLiteral("7tv"),
             QStringLiteral("shared-id"), 1, QStringLiteral("kick/b")),
        item(QStringLiteral("SameName"), QStringLiteral("twitch"),
             QStringLiteral("25"), 0, QStringLiteral("twitch/a")),
        item(QStringLiteral("SameName"), QStringLiteral("kick"),
             QStringLiteral("9001"), 1, QStringLiteral("kick/b")),
    };

    const auto merged = mergeMultiChannelEmoteItems(std::move(items));
    ASSERT_EQ(merged.size(), 3);
    EXPECT_EQ(merged[0].availableInChildren,
              (std::vector<size_t>{0, 1}));
    EXPECT_EQ(merged[0].availabilityContexts,
              (std::vector<QString>{QStringLiteral("twitch/a"),
                                    QStringLiteral("kick/b")}));
    const auto sharedCandidate = draftCandidateFromEmoteItem(merged[0]);
    ASSERT_TRUE(sharedCandidate.has_value());
    EXPECT_EQ(sharedCandidate->availabilityAlternatives.size(), 2U);
    EXPECT_EQ(std::ranges::count_if(merged, [](const auto &candidate) {
                  return candidate.displayName == QStringLiteral("SameName");
              }),
              2);
}

TEST(MultiChannelEmoteCompletion, DraftNarrowsChildrenAndRankingUsesTiers)
{
    const auto item = [](QString name, QString provider, QString id,
        ...
        for (const auto child : children)
        {
            availabilities.push_back({
                .platform = child == 0 ? QStringLiteral("twitch")
                                       : QStringLiteral("kick"),
                .channelID = child == 0 ? QStringLiteral("a")
                                        : QStringLiteral("b"),
            });
        }
        return EmoteItem{
            ...
            .availabilities = std::move(availabilities),
            .availableInChildren = std::move(children),
        };
    };
    const std::vector items{
        item(QStringLiteral("TwitchOnly"), QStringLiteral("twitch"),
             QStringLiteral("25"), {0}),
        item(QStringLiteral("KickOnly"), QStringLiteral("kick"),
             QStringLiteral("9001"), {1}),
        item(QStringLiteral("Shared"), QStringLiteral("7tv"),
             QStringLiteral("shared"), {0, 1}),
    };
    const std::vector destinations{
        MessageSendContext{
            .platform = QStringLiteral("twitch"),
            .channelID = QStringLiteral("a"),
            .accountID = QStringLiteral("ta"),
            .writable = true,
            .authenticated = true,
        },
        MessageSendContext{
            .platform = QStringLiteral("kick"),
            .channelID = QStringLiteral("b"),
            .accountID = QStringLiteral("ka"),
            .writable = true,
            .authenticated = true,
        },
    };

    auto all = filterMultiChannelEmoteItems(
        items, MessageDraft::fromPlainText({}), destinations, 1);
    prioritizeMultiChannelEmoteItems(all);
    ASSERT_EQ(all.size(), 3);
    EXPECT_EQ(all[0].displayName, QStringLiteral("Shared"));
    EXPECT_EQ(all[0].compatibleChannelCount, 2);
    EXPECT_EQ(all[1].displayName, QStringLiteral("KickOnly"));
    EXPECT_TRUE(all[1].primaryChannelCompatible);
    EXPECT_EQ(all[2].displayName, QStringLiteral("TwitchOnly"));
    EXPECT_FALSE(all[2].primaryChannelCompatible);

    const auto selected = draftCandidateFromEmoteItem(items[0]);
    ASSERT_TRUE(selected.has_value());
    auto draft = MessageDraft::reconstruct(QStringLiteral("TwitchOnly "),
                                           std::span{&*selected, 1U});
    auto narrowed =
        filterMultiChannelEmoteItems(items, draft, destinations, 1);
    prioritizeMultiChannelEmoteItems(narrowed);
    ASSERT_EQ(narrowed.size(), 2);
    EXPECT_EQ(narrowed[0].displayName, QStringLiteral("TwitchOnly"));
    EXPECT_EQ(narrowed[1].displayName, QStringLiteral("Shared"));
    EXPECT_TRUE(std::ranges::none_of(narrowed, [](const auto &candidate) {
        return candidate.displayName == QStringLiteral("KickOnly");
    }));

    auto typedPlain = filterMultiChannelEmoteItems(
        items, MessageDraft::fromPlainText(QStringLiteral("TwitchOnly ")),
        destinations, 1);
    EXPECT_EQ(typedPlain.size(), 3U);

    // An explicit platform selection is stronger than an earlier emote
    // selection: existing rendered text no longer vetoes the platform, while
    // new completion candidates are restricted to that platform's children.
    draft.destinationPlatformOverride = QStringLiteral("kick");
    auto kickOverride =
        filterMultiChannelEmoteItems(items, draft, destinations, 1);
    prioritizeMultiChannelEmoteItems(kickOverride);
    ASSERT_EQ(kickOverride.size(), 2U);
    EXPECT_EQ(kickOverride[0].displayName, QStringLiteral("KickOnly"));
    EXPECT_EQ(kickOverride[1].displayName, QStringLiteral("Shared"));
    EXPECT_TRUE(std::ranges::none_of(kickOverride, [](const auto &candidate) {
        return candidate.displayName == QStringLiteral("TwitchOnly");
    }));
    EXPECT_TRUE(std::ranges::all_of(kickOverride, [](const auto &candidate) {
        return candidate.compatibleChannelCount == 1 &&
               candidate.primaryChannelCompatible;
    }));

    auto unavailable = destinations;
    unavailable[1].authenticated = false;
    EXPECT_TRUE(
        filterMultiChannelEmoteItems(items, draft, unavailable, 1).empty());
}
