...
Allows a plugin to contribute one bounded, Chatterino-rendered control tab to
the top of supported main and popup windows. The permission lets the plugin
publish text and state for native status rows, actions, toggles, and choices,
and receive interactions with those declared controls. It does not expose Qt
widgets, window handles, layout access, custom painting, HTML, scripts, file
paths, URLs, or web content.
...
#### `c2.ui.register_control_tab(descriptor)`

Requires the [`UiControlTabs`](#uicontroltabs) permission. Registers the one
top control tab owned by the current plugin generation and returns a
`PluginControlTabHandle`. Different plugins can contribute tabs at the same
time; Chatterino orders them by plugin ID and compacts excess tabs into a
host-owned “More…” menu. A second registration by the same generation fails
without changing the first registration.

Chatterino owns all widgets, layout, rendering, accessibility, scaling,
keyboard behavior, focus, compaction, and lifetime. A contribution is visibly
identified with the plugin's metadata name. It is removed when the plugin is
disabled, unloaded, reloaded, fails during initial loading, or calls
`handle:remove()`. Old handles cannot affect a later plugin generation.

The descriptor accepts:

- `title` (required): collapsed-tab title.
- `icon`: one of `play`, `pause`, `stop`, `refresh`, `settings`, `info`,
  `warning`, `error`, `check`, or `status`. Omit it for no icon. Image data,
  paths, and URLs are rejected.
- `tooltip` and `accessible_label`: optional plain text.
- `visible`: optional initial visibility, defaulting to `true`.
- `summary`: optional collapsed state with `primary_text`, `secondary_text`,
  numeric `value`, `units`, `severity`, `stale`, and `open_enabled`.
- `controls`: an optional array of the host-rendered controls described below.
- `on_control(id, proposed_value)`: required when any action, toggle, or choice
  exists. Actions pass only `id`; toggles pass a boolean; choices pass their
  option value.

Valid summary severities are `neutral`, `active`, `success`, `warning`,
`error`, and `unknown`. The host exposes severity and stale state through text
and accessibility rather than relying on color alone.

The control vocabulary is deliberately small:

- `status`: read-only `text`, numeric `value`, `units`, optional `progress`
  from 0 through 1, and `stale` state.
- `action`: a momentary native button.
- `toggle`: a native checkbox with a boolean `value`.
- `choice`: a native single-choice control with a selected string `value` and
  an `options` array of `{ value, label }` entries.

Every control has a stable plugin-local `id`, `type`, and `label`, plus
optional `description`, accepted `icon`, `visible`, `enabled`, and `pending`
fields. `pending = true` disables interaction and tells accessibility clients
that the plugin has not yet confirmed an operation.

Control interactions are intents, not optimistic state changes. The plugin
performs its operation and calls `update_controls` with the authoritative
result. If a callback fails, Chatterino logs the plugin-local error and restores
the last valid host state. Reentrant callbacks are rejected.

##### Static status example

This is useful for a plugin that only publishes information. No callback is
needed because every row is read-only.

```lua
local status_tab = c2.ui.register_control_tab({
    title = "Build monitor",
    icon = "status",
    tooltip = "State of local background builds",
...
-- update_summary replaces the complete summary atomically.
status_tab:update_summary({
    primary_text = "Building",
    secondary_text = "Compiling the application",
    value = 2,
    units = "jobs",
    severity = "active",
})
...
##### Interactive generic-control example

This example is intentionally unrelated to playback. It demonstrates every
interactive control type and waits for its own operation before publishing the
...
local function on_control(id, proposed)
    if id == "refresh" then
        tab:update_controls({ { id = id, pending = true } })
        refresh_local_data(function(ok, new_count)
            tab:update_controls({
                { id = id, pending = false },
                { id = "items", value = new_count },
            })
            tab:update_summary({
                primary_text = ok and "Current" or "Refresh failed",
                severity = ok and "success" or "error",
            })
        end)
    elseif id == "notifications" then
        set_notifications_enabled(proposed, function(confirmed)
            tab:update_controls({
                { id = id, value = confirmed, pending = false },
            })
        end)
    elseif id == "profile" then
        select_profile(proposed, function(confirmed_profile)
            tab:update_controls({
                { id = id, value = confirmed_profile, pending = false },
            })
        end)
    end
end
...
tab = c2.ui.register_control_tab({
    ...
    icon = "settings",
    controls = {
        { id = "items", type = "status", label = "Items", value = 0 },
        { id = "refresh", type = "action", label = "Refresh", icon = "refresh" },
        {
            id = "notifications",
            type = "toggle",
            label = "Notifications",
...
##### Dynamic follower consumer

The stream follower remains an external process. Its Chatterino plugin can use
the same generic API without core knowing about Streamlink or mpv: the plugin
maps sanitized bridge status into the descriptor and sends control intents
back over its existing local transport.

```lua
local player_tab

local function publish_player_status(status)
    player_tab:update_summary({
        primary_text = status.lifecycle,
        secondary_text = status.platform .. " playback",
        value = status.lag_seconds,
        units = "s lag",
        severity = status.stale and "unknown" or "active",
        stale = status.stale,
    })
    player_tab:update_controls({
        { id = "playback", text = status.lifecycle, stale = status.stale },
        { id = "follow", value = status.follow_enabled, pending = status.pending },
        { id = "quality", value = status.quality, pending = status.pending },
    })
end

player_tab = c2.ui.register_control_tab({
    title = "Follower",
    icon = "play",
    controls = {
        { id = "playback", type = "status", label = "Playback", text = "Unknown", stale = true },
        { id = "follow", type = "toggle", label = "Follow selected tab", value = true },
        {
            id = "quality",
            type = "choice",
            label = "Quality",
    ...
    on_control = function(id, proposed)
        player_tab:update_controls({ { id = id, pending = true } })
        follower_bridge_send({ control = id, value = proposed })
    end,
})
```

##### Handle methods and bounds

- `handle:update_summary(summary)` atomically replaces the collapsed summary.
- `handle:update_controls(states)` atomically applies partial authoritative
  states by control ID. Use `clear_value` or `clear_progress` on a status row
  to clear those optional numeric fields.
- `handle:set_visible(visible)` shows or hides the contribution.
- `handle:remove()` removes it permanently for this handle.
- `handle:is_valid()` reports whether this generation still owns the tab.

Invalid descriptors and updates raise a Lua error and preserve the last valid
state. The current host bounds are 12 tabs application-wide, 16 controls per
tab, 16 options per choice, 48 characters for titles and IDs, 80 for labels,
96/160 for primary/secondary status, 200 for accessible labels, and 240 for
descriptions/tooltips. Numeric values must be finite and within ±1e12.
State-only rendering is coalesced to at most once per Qt event-loop turn, so a
plugin cannot force an unbounded repaint loop. Unknown fields are rejected;
plain strings are never interpreted as HTML, stylesheets, scripts, paths, or
URLs.
