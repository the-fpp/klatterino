...
void PluginController::initSol(sol::state_view &lua, Plugin *plugin)
{
    ...
    lua::api::createControlTabUserTypes(c2, plugin);
                            ...
                            const PluginMeta &meta)
{
    ...
    if (err != 0)
    {
        ...
        if (temp->controlTab())
        {
            temp->removeControlTab(temp->controlTab().get());
            this->notifyControlTabsChanged();
        }
        qCWarning(chatterinoLua)
...
bool PluginController::tryHandleEmptyInputBackspace(Split &split) const
{
    for (const auto &[name, plugin] : this->plugins_)
    {
        (void)name;
        if (!plugin || !plugin->error().isNull() || plugin->state_ == nullptr)
        {
            continue;
        }

        auto it = plugin->callbacks.find(lua::api::EventType::EmptyInputBackspace);
        if (it == plugin->callbacks.end())
        {
            continue;
        }

        sol::state_view lua(plugin->state_);
        sol::table args = lua.create_table_with("split", QPointer(&split));

        auto result = lua::tryCall<std::optional<bool>>(it->second, args);
        if (!result)
        {
            qCDebug(chatterinoLua)
                << "Got error from plugin" << plugin->meta.name
                << "while handling empty input Backspace:" << result.error();
            continue;
        }

        if (result.value().value_or(false))
        {
            return true;
        }
    }
...
std::vector<PluginControlTabSnapshot> PluginController::controlTabs() const
{
    std::vector<PluginControlTabSnapshot> result;
    result.reserve(this->plugins_.size());
    for (const auto &[_, plugin] : this->plugins_)
    {
        const auto registration = plugin->controlTab();
        if (registration && registration->isActive())
        {
            result.push_back(registration->snapshot());
        }
    }
    return result;
}

std::size_t PluginController::activeControlTabCount() const
{
    return std::ranges::count_if(this->plugins_, [](const auto &entry) {
        const auto registration = entry.second->controlTab();
        return registration && registration->isActive();
    });
}

Expected<void, QString> PluginController::invokeControlTab(
    const QString &pluginID, const QString &controlID,
    const PluginControlValue &proposedValue)
{
    const auto plugin = this->plugins_.find(pluginID);
    if (plugin == this->plugins_.end())
    {
        return makeUnexpected(
            QStringLiteral("control-tab owner is no longer loaded"));
    }
    const auto registration = plugin->second->controlTab();
    if (!registration || !registration->isActive())
    {
        return makeUnexpected(
            QStringLiteral("control tab is no longer registered"));
    }
    return registration->invoke(controlID, proposedValue);
}

void PluginController::notifyControlTabsChanged()
{
    this->onControlTabsUpdated.invoke();
}

void PluginController::queueControlTabUpdate()
{
    if (this->controlTabUpdateQueued_)
    {
        return;
    }
    this->controlTabUpdateQueued_ = true;
    QMetaObject::invokeMethod(
        qApp,
        [this] {
            this->controlTabUpdateQueued_ = false;
            this->onControlTabsUpdated.invoke();
        },
        Qt::QueuedConnection);
}
