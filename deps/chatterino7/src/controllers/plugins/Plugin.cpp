...
Plugin::~Plugin()
{
    if (this->controlTab_)
    {
        this->controlTab_->detach();
        this->controlTab_.reset();
    }
...
{
    return this->hasPermission(PluginPermission::Type::Network);
}

bool Plugin::hasPermission(PluginPermission::Type type) const
{
    return std::ranges::any_of(this->meta.permissions, [type](const auto &p) {
        return p.type == type;
    });
}

std::shared_ptr<lua::api::PluginControlTabRegistration> Plugin::controlTab()
    const
{
    return this->controlTab_;
}

void Plugin::setControlTab(
    std::shared_ptr<lua::api::PluginControlTabRegistration> registration)
{
    assert(!this->controlTab_);
    this->controlTab_ = std::move(registration);
}

void Plugin::removeControlTab(
    const lua::api::PluginControlTabRegistration *registration)
{
    if (this->controlTab_.get() != registration)
    {
        return;
    }
    this->controlTab_->detach();
