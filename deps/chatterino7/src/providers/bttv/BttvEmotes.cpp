...
std::pair<Outcome, EmoteMap> parseGlobalEmotesWithOutcome(
...
{
    auto parsed = parseGlobalEmotesWithOutcome(jsonEmotes, currentEmotes);
...
void BttvEmotes::loadEmotes()
{
    ...
    readProviderEmotesCache("global", "betterttv", [this](const auto &jsonDoc) {
        auto emotes = this->global_.get();
        auto pair = parseGlobalEmotesWithOutcome(jsonDoc.array(), *emotes);
        if (pair.first)
        {
    ...
    NetworkRequest(QString(globalEmoteApiUrl))
        ...
        .onSuccess([this](auto result) {
            ...
            auto emotes = this->global_.get();
            auto pair =
                parseGlobalEmotesWithOutcome(result.parseJsonArray(), *emotes);
            if (pair.first)
            {
                this->setEmotes(
