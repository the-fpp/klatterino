...
void NetworkConfigurationProvider::applyFromEnv(const Env &env)
{
    ...
    if (env.sslCertFile)
    {
        if (applyCertificateBundle(*env.sslCertFile))
        {
            qCInfo(chatterinoNetwork)
                << "Loaded the configured TLS certificate bundle.";
        }
        else
        {
            qCWarning(chatterinoNetwork)
                << "The configured TLS certificate bundle could not be loaded.";
        }
    }
}
