...
MessageSendContext TwitchChannel::messageSendContext() const
{
    auto context = Channel::messageSendContext();
    const auto account = getApp()->getAccounts()->twitch.getCurrent();
    context.authenticated = !account->isAnon();
    context.accountID = account->getUserId();
    context.writable = context.writable && !this->disconnected_;
    ...
    appendDraftEmoteCapabilities(context.emoteCapabilities,
                                 *this->localTwitchEmotes(),
                                 QStringLiteral("twitch"),
                                 channelAccountScope);
    appendDraftEmoteCapabilities(context.emoteCapabilities,
                                 **account->accessEmotes(),
                                 QStringLiteral("twitch"), accountScope);
    if (const auto *personal = getApp()->getSeventvPersonalEmotes())
    {
        for (const auto &map :
             personal->getEmoteSetsForTwitchUser(context.accountID))
        {
            appendDraftEmoteCapabilities(context.emoteCapabilities, *map,
                                         QStringLiteral("7tv"), accountScope);
        }
    }
    appendDraftEmoteCapabilities(context.emoteCapabilities,
                                 *this->bttvEmotes(),
                                 QStringLiteral("bttv"), channelScope);
    appendDraftEmoteCapabilities(context.emoteCapabilities,
                                 *this->ffzEmotes(), QStringLiteral("ffz"),
                                 channelScope);
    appendDraftEmoteCapabilities(context.emoteCapabilities,
                                 *this->seventvEmotes(),
                                 QStringLiteral("7tv"), channelScope);
    if (const auto *provider = getApp()->getBttvEmotes())
    {
        if (const auto global = provider->emotes())
        {
            appendDraftEmoteCapabilities(context.emoteCapabilities, *global,
                                         QStringLiteral("bttv"),
                                         platformScope);
        }
    }
    if (const auto *provider = getApp()->getFfzEmotes())
    {
        if (const auto global = provider->emotes())
        {
            appendDraftEmoteCapabilities(context.emoteCapabilities, *global,
    ...
    if (const auto *provider = getApp()->getSeventvEmotes())
    {
        if (const auto global = provider->globalEmotes())
        {
            appendDraftEmoteCapabilities(context.emoteCapabilities, *global,
                                         QStringLiteral("7tv"),
                                         platformScope);
    ...
    if (const auto *emotes = getApp()->getEmotes())
    {
        appendDraftEmojiCapabilities(
            context.emoteCapabilities,
            emotes->getEmojis()->getEmojis());
    }
