...
MessageSendContext KickChannel::messageSendContext() const
{
    auto context = Channel::messageSendContext();
    const auto account = getApp()->getAccounts()->kick.current();
    context.authenticated = !account->isAnonymous() && this->userID_ != 0;
    context.writable = context.writable && this->userID_ != 0;
    context.accountID = QString::number(account->userID());
    ...
    {
        for (const auto &map :
             personal->getEmoteSetsForKickUser(account->userID()))
        {
    ...
    if (const auto *server = getApp()->getKickChatServer())
    {
        appendDraftEmoteCapabilities(context.emoteCapabilities,
                                     *server->globalEmotes(),
                                     QStringLiteral("kick"), platformScope);
    }
    ...
    KickApi::privateChannelHistory(
        this->channelID_, [weak = this->weakFromThis()](const auto &res) {
            ...
            for (auto i = static_cast<qsizetype>(arr.size() - 1); i >= 0; --i)
            {
                ...
                if (ptr)
                {
                    ptr->flags.set(MessageFlag::RecentMessage);
