...
void loadKickHistoryAttempt(uint64_t channelID,
                            KickApi::Callback<BoostJsonObject> cb, int attempt)
{
    autoSlugify<BoostJsonObject>(
        u"https://web.kick.com/api/v1/chat/" % QString::number(channelID) %
            "/history",
        [channelID, cb = std::move(cb), attempt](auto result) mutable {
            if (!result && attempt < MAX_HISTORY_LOAD_ATTEMPTS)
            {
                QTimer::singleShot(
                    HISTORY_RETRY_DELAY_MS * attempt,
                    [channelID, cb = std::move(cb), attempt]() mutable {
                        loadKickHistoryAttempt(channelID, std::move(cb),
                                               attempt + 1);
                    });
                return;
            }
            ...
            const QString error = loaded ? QString{} : result.error();
            auto channel =
                getApp()->getKickChatServer()->findByChannelID(channelID);
            ...
            if (channel)
            {
                if (loaded)
                {
                    messagehistory::Registry::instance().setLoaded(channel);
                }
                else
                {
                    messagehistory::Registry::instance().setFailed(channel,
                                                                   error);
                }
            }
        });
                                    ...
                                    Callback<BoostJsonObject> cb)
{
    ...
    loadKickHistoryAttempt(channelID, std::move(cb), 1);
}
