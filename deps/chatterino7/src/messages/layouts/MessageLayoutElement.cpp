...
bool allowsSevenTVPaint(const chatterino::MessageElement &element)
{
    return !element.getFlags().has(
        chatterino::MessageElementFlag::RumbleUsername);
}
                              ...
                              const MessageColors & /*messageColors*/)
{
    ...
    bool drawPaint = isNametag && allowsSevenTVPaint(this->getCreator()) &&
...
bool TextLayoutElement::paintAnimated(QPainter &painter, const qreal yOffset)
{
    ...
    const bool drawPaint = isNametag &&
                           allowsSevenTVPaint(this->getCreator()) &&
                           getSettings()->displaySevenTVPaints;
    if (!drawPaint)
    {
        return false;
