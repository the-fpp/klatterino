    ...
    switch (channel.getType())
    {
        ...
        case Channel::Type::Rumble:
            if (const auto &identity = channel.layoutIdentity();
                identity && identity->platform == QStringLiteral("rumble") &&
                !identity->locator.isEmpty())
            {
                // Revalidate at the final serialization boundary. A malformed
                // or unsafe imported value stays a typed placeholder but is
                // never written back to disk.
                auto locator =
                    RumbleLayoutLocator::fromPersisted(identity->locator);
                if (locator)
                {
                    obj.insert("locator", locator->canonicalUrl());
                    obj.insert("layoutIdentity",
                               QJsonObject{
