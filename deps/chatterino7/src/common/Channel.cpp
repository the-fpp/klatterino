    ...
    , type_(type)
{
    ...
    else if (this->isRumbleChannel())
    {
        this->messagePlatform_ = MessagePlatform::Rumble;
    }
    else
    {
...
bool Channel::isRumbleChannel() const
{
    return this->type_ == Type::Rumble;
}
...
bool Channel::isWritable() const
{
    ...
    return type != Type::TwitchMentions && type != Type::TwitchLive &&
           type != Type::TwitchAutomod && type != Type::Rumble;
}

MessageSendContext Channel::messageSendContext() const
{
    QString platform;
    if (this->isTwitchChannel())
    {
    ...
    return {
        ...
        .authenticated = this->canSendMessage(),
    };
}

MessageDraftEvaluation Channel::evaluateMessageDraft(
    const MessageDraft &draft) const
{
    return chatterino::evaluateMessageDraft(draft,
                                             this->messageSendContext());
}
...
void Channel::sendMessageAsync(QString message, SendCallback callback)
{
    // Compatibility adapter for providers that implement only the established
    // synchronous sendMessage() contract. Exactly one virtual dispatch occurs;
    // providers with observable asynchronous outcomes override this method.
...
// Indirect channel
//
IndirectChannel::Data::Data(
    ChannelPtr _channel, Channel::Type _type,
    std::optional<ChannelLayoutIdentity> _layoutIdentity)
    : channel(std::move(_channel))
    , type(_type)
    , layoutIdentity(std::move(_layoutIdentity))
{
}

IndirectChannel::IndirectChannel(
    ChannelPtr channel, Channel::Type type,
    std::optional<ChannelLayoutIdentity> layoutIdentity)
    : data_(std::make_unique<Data>(std::move(channel), type,
                                   std::move(layoutIdentity)))
{
}
...
const std::optional<ChannelLayoutIdentity> &IndirectChannel::layoutIdentity()
    const
{
    return this->data_->layoutIdentity;
}

void IndirectChannel::reconnect() const
{
    if (this->data_->layoutIdentity &&
        this->data_->layoutIdentity->platform == QStringLiteral("rumble"))
    {
        if (auto *app = tryGetApp())
        {
            if (auto *rumble = app->getRumble())
            {
                rumble->retry(*this);
                return;
            }
