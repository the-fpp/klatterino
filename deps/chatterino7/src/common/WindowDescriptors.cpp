...
std::optional<ChannelLayoutIdentity> loadLayoutIdentity(const QJsonObject &obj)
{
    const auto value = obj.value(QStringLiteral("layoutIdentity"));
    if (!value.isObject())
    {
        return std::nullopt;
    }
    const auto identity = value.toObject();
    const auto platform = identity.value(QStringLiteral("platform")).toString();
    const auto locator = identity.value(QStringLiteral("locator")).toString();
    if (platform.isEmpty())
    {
        return std::nullopt;
    }
    return ChannelLayoutIdentity{
        .platform = platform,
        .locator = locator,
    };
}

QJsonObject encodeLayoutIdentity(const ChannelLayoutIdentity &identity)
{
    QJsonObject result{
        {QStringLiteral("platform"), identity.platform},
    };
    if (!identity.locator.isEmpty())
    {
        result.insert(QStringLiteral("locator"), identity.locator);
    }
    return result;
}

ChannelLayoutIdentity loadRumbleIdentity(const QJsonObject &obj)
{
    auto candidate = loadLayoutIdentity(obj);
    QString input;
    if (candidate && candidate->platform == QStringLiteral("rumble"))
    {
    ...
    if (!locator)
    {
        locator = RumbleLayoutLocator::fromPersisted(
            obj.value(QStringLiteral("locator")).toString());
    }
    return {
        .platform = QStringLiteral("rumble"),
        .locator = locator ? locator->canonicalUrl() : QString{},
    };
}
...
{
    const auto platform = obj["platform"].toString();
    auto channelName = obj["channel"].toString();
    std::optional<ChannelLayoutIdentity> identity;
    if (platform.compare(QStringLiteral("Rumble"), Qt::CaseInsensitive) == 0)
    {
        identity = loadRumbleIdentity(obj);
        channelName.clear();
    }
    return {
        .platform = platform,
        .channelName = std::move(channelName),
        .layoutIdentity = std::move(identity),
    };
}

QJsonObject ChildChannelDescriptor::toJson() const
{
    const bool rumble = this->platform.compare(QStringLiteral("Rumble"),
                                               Qt::CaseInsensitive) == 0;
    QJsonObject result{{QLatin1StringView("platform"), this->platform}};
    if (!rumble)
    {
        result.insert(QLatin1StringView("channel"), this->channelName);
    }
    if (this->layoutIdentity)
    {
        if (rumble)
        {
            auto locator = RumbleLayoutLocator::fromPersisted(
                this->layoutIdentity->locator);
            if (this->layoutIdentity->platform == QStringLiteral("rumble") &&
                locator)
            {
                const ChannelLayoutIdentity canonical{
                    .platform = QStringLiteral("rumble"),
                    .locator = locator->canonicalUrl(),
                };
                result.insert(QStringLiteral("layoutIdentity"),
                              encodeLayoutIdentity(canonical));
                result.insert(QStringLiteral("locator"), canonical.locator);
            }
        }
    }
    return result;
                                   ...
                                   const QJsonObject &data)
{
    ...
    if (descriptor.type_ == u"rumble")
    {
        descriptor.layoutIdentity = loadRumbleIdentity(data);
        descriptor.channelName_.clear();
    }
...
IndirectChannel SplitDescriptor::decodeChannel() const
{
    ...
    switch (*type)
    {
        ...
        case Channel::Type::Rumble:
            ...
            {
                const auto locator =
                    this->layoutIdentity && this->layoutIdentity->platform ==
                                                QStringLiteral("rumble")
                        ? this->layoutIdentity->locator
                        : QString{};
                return controller->restore(locator);
            }
