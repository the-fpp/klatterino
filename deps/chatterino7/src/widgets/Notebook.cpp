                                 ...
                                 this))
{
    ...
    this->addButton_->setObjectName(QStringLiteral("notebookAddTab"));
    this->addButton_->setAccessibleName(QStringLiteral("Add tab"));
...
void Notebook::selectNextTab(bool focusPage)
{
    ...
    if (!this->tabVisibilityFilter_)
    {
        if (startIndex < 0)
        {
            this->select(this->items_.front().page, focusPage);
            return;
        }
        if (size == 1)
        {
...
void Notebook::selectPreviousTab(bool focusPage)
{
    ...
    if (!this->tabVisibilityFilter_)
    {
        if (startIndex < 0)
        {
            this->select(this->items_.back().page, focusPage);
            return;
        }
        if (size == 1)
        {
...
void Notebook::selectNextLiveTab(bool focusPage)
{
    this->selectAdjacentLiveTab(true, focusPage);
}

void Notebook::selectPreviousLiveTab(bool focusPage)
{
    this->selectAdjacentLiveTab(false, focusPage);
}

void Notebook::selectAdjacentLiveTab(bool forward, bool focusPage)
{
    const int size = this->items_.size();
    if (size == 0)
    {
    ...
    const auto eligible = [this](const Item &item) {
        return item.tab->isLive() &&
               (!this->tabVisibilityFilter_ ||
                this->tabVisibilityFilter_(item.tab));
    };
    const auto liveCount = static_cast<int>(
        std::ranges::count_if(this->items_, eligible));
    if (liveCount < 2)
    {
        return;
    }
    ...
    if (startIndex < 0)
    {
        ...
        {
            for (auto it = this->items_.rbegin();
                 it != this->items_.rend(); ++it)
            {
                if (eligible(*it))
                {
    ...
    {
        index = forward ? (index + 1) % size
                        : (index == 0 ? size - 1 : index - 1);
        if (eligible(this->items_[index]))
        {
            this->select(this->items_[index].page, focusPage);
            return;
        }
...
bool Notebook::isCompactMode() const
{
    return this->compactMode_;
}

void Notebook::setCompactMode(bool compact)
{
    if (this->compactMode_ == compact)
    {
        return;
    }

    this->compactMode_ = compact;
    const bool revealCleared = !compact && this->compactTabsRevealed_;
    if (revealCleared)
    {
        this->compactTabsRevealed_ = false;
        qApp->removeEventFilter(this);
    }

    // Preserve showTabs_ and showAddButton_: they represent the user's chosen
    // presentation and must be restored verbatim when compact mode ends.
    this->addButton_->setHidden(
        !this->showAddButton_ ||
        (this->compactMode_ && !this->compactTabsRevealed_));
    this->updateCompactTabControls();
    this->performLayout();
    this->updateTabVisibility();
    if (revealCleared)
    {
        Q_EMIT this->compactTabPresentationChanged();
    }
}

bool Notebook::areCompactTabsRevealed() const
{
    return this->compactTabsRevealed_;
}

void Notebook::setCompactTabsRevealed(bool revealed)
{
    revealed = revealed && this->compactMode_;
    if (revealed == this->compactTabsRevealed_)
    {
        return;
    }

    this->compactTabsRevealed_ = revealed;
    if (revealed)
    {
...
CompactTabStatus Notebook::getCompactTabStatus() const
{
    return this->compactTabStatus_;
}

const QString &Notebook::getCompactSelectedTitle() const
{
    return this->compactSelectedTitle_;
}
...
void Notebook::setShowAddButton(bool value)
{
    ...
    this->addButton_->setHidden(
        !value || (this->compactMode_ && !this->compactTabsRevealed_));

    this->refresh();
}

void Notebook::addCompactTabControls()
{
    if (this->compactTabControls_.complete())
    {
        return;
    }

    this->compactTabControls_.previousLive =
        this->addCustomButton<LabelButton>(QString());
    this->compactTabControls_.previous =
        this->addCustomButton<LabelButton>(QString());
    this->compactTabControls_.status =
        this->addCustomButton<LabelButton>(QString());
    this->compactTabControls_.next =
        this->addCustomButton<LabelButton>(QString());
    this->compactTabControls_.nextLive =
        this->addCustomButton<LabelButton>(QString());

    initializeCompactTabControlButtons(this->compactTabControls_);
    QObject::connect(this->compactTabControls_.previous,
                     &LabelButton::leftClicked, this, [this] {
                         this->selectPreviousTab();
                     });
    QObject::connect(this->compactTabControls_.next, &LabelButton::leftClicked,
                     this, [this] {
                         this->selectNextTab();
                     });
    QObject::connect(this->compactTabControls_.previousLive,
                     &LabelButton::leftClicked, this, [this] {
                         this->selectPreviousLiveTab();
                     });
    QObject::connect(this->compactTabControls_.nextLive,
                     &LabelButton::leftClicked, this, [this] {
                         this->selectNextLiveTab();
                     });
    QObject::connect(this->compactTabControls_.status,
                     &LabelButton::leftClicked, this, [this] {
                         this->setCompactTabsRevealed(true);
                     });

    this->updateCompactTabControls();
}

bool Notebook::shouldShowNormalTabs() const
{
    return this->compactMode_ ? this->compactTabsRevealed_ : this->showTabs_;
}

int Notebook::customButtonWidth(const Button *button,
                                const LayoutContext &ctx) const
{
    if (button == this->compactTabControls_.status)
    {
        // setFixedSize() constrains QWidget::sizeHint(), so measuring the text
        // directly lets the status grow when the count gains another digit.
        const auto *status = this->compactTabControls_.status;
        const auto textWidth =
            status->fontMetrics().horizontalAdvance(status->text());
        return std::max(ctx.buttonWidth,
                        textWidth + status->padding().width() * 2);
    }
    return ctx.buttonWidth;
}

void Notebook::updateCompactTabStatus()
{
    std::vector<CompactTabEntry> tabs;
    tabs.reserve(this->items_.size());
    for (const auto &item : this->items_)
    {
        tabs.push_back({
            .navigable = !this->tabVisibilityFilter_ ||
                         this->tabVisibilityFilter_(item.tab),
            .live = item.tab->isLive(),
        });
    }

    const auto selected = this->getSelectedIndex();
    const auto selectedIndex =
        selected < 0 ? std::optional<size_t>{}
                     : std::optional<size_t>{static_cast<size_t>(selected)};
    const auto status = compactTabStatus(tabs, selectedIndex);
    const auto title = selected >= 0 && selected < this->items_.size()
                           ? this->items_[selected].tab->getTitle()
                           : QString();

    const bool changed =
        status.selectedPosition != this->compactTabStatus_.selectedPosition ||
        status.navigableCount != this->compactTabStatus_.navigableCount ||
        status.liveNavigableCount !=
            this->compactTabStatus_.liveNavigableCount ||
        status.controlsEnabled != this->compactTabStatus_.controlsEnabled ||
        status.liveControlsEnabled !=
            this->compactTabStatus_.liveControlsEnabled ||
        title != this->compactSelectedTitle_;

    this->compactTabStatus_ = status;
    this->compactSelectedTitle_ = title;
    this->updateCompactTabControls();

    if (changed)
    {
        Q_EMIT this->compactTabStatusChanged();
    }
}

void Notebook::updateCompactTabControls()
{
    applyCompactTabControlState(
        this->compactTabControls_, this->compactTabStatus_,
        this->compactSelectedTitle_,
        this->compactMode_ && !this->compactTabsRevealed_);
}
...
void Notebook::scaleChangedEvent(float /*scale*/)
{
    ...
    // Custom compact controls also scale, including the empty-tab state where
    // no NotebookTab::updateSize call can request a refresh.
    if (this->refreshRequested_ || this->compactTabControls_.complete())
    {
        this->refresh();
    }
}
...
bool Notebook::eventFilter(QObject *watched, QEvent *event)
{
    if (!this->compactTabsRevealed_)
    {
    ...
    // On a real platform backend Qt can initially filter the pointer event for
    // the top-level window rather than the eventual child widget. Hit-test the
    // visible tab geometry instead of relying on QObject parentage so the
    // press still reaches NotebookTab and performs its normal selection.
    const auto globalPosition =
        static_cast<QMouseEvent *>(event)->globalPosition().toPoint();
    const bool overTab =
        std::ranges::any_of(this->items_, [&](const auto &item) {
            return item.tab->isVisible() &&
                   item.tab->rect().contains(
                       item.tab->mapFromGlobal(globalPosition));
        });
    const bool overAddButton =
        this->addButton_->isVisible() &&
        this->addButton_->rect().contains(
            this->addButton_->mapFromGlobal(globalPosition));
    const bool overTabControl = overTab || overAddButton;

    if (type == QEvent::MouseButtonPress && !overTabControl)
    {
        this->setCompactTabsRevealed(false);
    }
...
{
    this->updateCompactTabStatus();

    // When the compact controls live in the native custom titlebar there is no
    // notebook-hosted chrome to reserve space for.
    if (this->compactMode_ && !this->compactTabsRevealed_ &&
        !this->compactTabControls_.complete())
    {
        this->lineOffset_ = 0;
        if (this->selectedPage_ != nullptr)
        {
            this->selectedPage_->setGeometry(this->rect());
    ...
    const LayoutContext ctx{
        ...
        .addButtonWidth = this->shouldShowNormalTabs() && this->showAddButton_
                              ? tabHeight
                              : 0,
...
void Notebook::performHorizontalLayout(const LayoutContext &ctx, bool animated)
{
    ...
    for (auto *btn : this->customButtons_)
    {
        ...
        const auto buttonWidth = this->customButtonWidth(btn, ctx);
        btn->setFixedSize(buttonWidth, ctx.buttonHeight);
        btn->move(x, y);
        x += buttonWidth;
...
void Notebook::performVerticalLayout(const LayoutContext &ctx, bool animated)
{
    ...
    int count = ctx.items.size() +
                (this->shouldShowNormalTabs() && this->showAddButton_ ? 1 : 0);
    ...
    : Notebook(parent)
{
    QObject::connect(this->addButton_, &Button::leftClicked, [this]() {
        const bool openCompactPicker =
            this->isCompactMode() && this->areCompactTabsRevealed();
        QTimer::singleShot(80, this, [this, openCompactPicker] {
            auto *page = this->addPage(true);
            if (openCompactPicker)
            {
...
void SplitNotebook::addCustomButtons()
{
    ...
    // Keep the compact group immediately adjacent to the account control.
