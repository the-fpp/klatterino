...
void Split::updateInputPlaceholder()
{
    ...
    if (auto *multiChannel = dynamic_cast<MultiChannel *>(channel.get()))
    {
        ...
        {
            this->input_->ui_.textEdit->setPlaceholderText(
                u"Send message in " % multiChannelChildDisplayName(*active) %
                u"...");
        }
        return;
    }
...
std::optional<QString> Split::getSelectedLocator() const
{
    std::optional<ChannelLayoutIdentity> identity;
    auto aggregate = this->channel_.get();
    if (const auto *multi = dynamic_cast<const MultiChannel *>(aggregate.get()))
    {
        const auto *active = multi->activeChannel();
        if (!active || active->platform != MultiChannel::Platform::Rumble ||
            !active->originalSpec.layoutIdentity ||
            active->originalSpec.layoutIdentity->platform !=
                QStringLiteral("rumble"))
        {
            return std::nullopt;
        }
        identity = active->originalSpec.layoutIdentity;
    }
    else
    {
        const auto selected = this->getSelectedChannel();
        if (!selected || !selected->isRumbleChannel() ||
            !this->channel_.layoutIdentity() ||
            this->channel_.layoutIdentity()->platform !=
                QStringLiteral("rumble"))
        {
            return std::nullopt;
        }
        identity = this->channel_.layoutIdentity();
    }
...
void Split::updateChannelConnections()
{
    ...
    if (mc)
    {
        ...
        channel = active ? active->channel.get() : nullptr;
    }
    if (!channel)
    {
        return;
...
{
    if (rumbleLayoutNeedsPickerRepair(this->channel_, getApp()->getRumble()))
    {
        // The picker is the only safe repair surface for an absent/unsafe
        // locator or an unavailable controller. Never reinterpret a runtime
        // UUID, and never advertise a placeholder reconnect that is a no-op.
        this->changeChannel();
        return;
    }
    this->channel_.reconnect();
}
