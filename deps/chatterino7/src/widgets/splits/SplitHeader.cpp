...
void SplitHeader::handleChannelChanged()
{
    ...
    else if (auto *rumbleChannel = dynamic_cast<RumbleChannel *>(channel.get()))
    {
        this->channelConnections_.managedConnect(
            rumbleChannel->streamTitleChanged, [this] {
                this->updateChannelText();
            });
        this->channelConnections_.managedConnect(
...
QString SplitHeader::channelText() const
{
...
void SplitHeader::updateChannelText()
{
    ...
    else if (auto *rumbleChannel =
                 dynamic_cast<RumbleChannel *>(selectedChannel.get()))
    {
        if (rumbleChannel->isLive())
        {
            this->isLive_ = true;
            title += QStringLiteral(" (live)");
            const auto streamTitle = rumbleChannel->streamTitle().simplified();
            if (getSettings()->headerStreamTitle && !streamTitle.isEmpty())
            {
                title += QStringLiteral(" - ") + streamTitle;
            }
            if (!streamTitle.isEmpty())
            {
                this->tooltipText_ =
                    QStringLiteral("<p style=\"text-align: center;\">%1</p>")
                        .arg(streamTitle.toHtmlEscaped());
            }
        }
        else
        {
            this->tooltipText_ = QStringLiteral("Offline");
        }
    }
