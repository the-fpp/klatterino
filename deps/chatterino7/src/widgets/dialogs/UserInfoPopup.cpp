...
ChannelPtr filterMessages(const UserCardIdentity &identity, ChannelPtr channel)
{
    ...
    {
        if (messageMatchesUserCard(identity, *message))
        {
    ...
    , closeAutomatically_(closeAutomatically)
{
    ...
    HotkeyController::HotkeyMap actions{
        ...
        {"execModeratorAction",
         [this](std::vector<QString> arguments) -> QString {
             if (!userCardSurfacePolicy(this->identity_).showModeratorActions)
             {
                 return "Moderation actions are unavailable for this provider";
             }
             if (arguments.empty())
             {
    ...
    auto head = layout.emplace<QHBoxLayout>().withoutMargin();
    {
        ...
        auto vbox = head.emplace<QVBoxLayout>();
        {
            ...
            // items on the left
            auto platformBox = vbox.emplace<QHBoxLayout>().withoutMargin();
            auto platformBadge =
                platformBox.emplace<PixmapButton>(nullptr).assign(
                    &this->ui_.platformBadge);
            platformBadge->setScaleIndependentSize(24, 24);
            platformBadge->setDim(DimButton::Dim::None);
            platformBadge->setEnabled(false);
            this->ui_.platformBadge->setObjectName(
                QStringLiteral("usercard-platform-badge"));
            platformBox.emplace<Label>().assign(&this->ui_.platformLabel);
            this->ui_.platformLabel->setObjectName(
                QStringLiteral("usercard-platform-label"));
            platformBox->addStretch(1);
            this->ui_.platformBadge->hide();
            this->ui_.platformLabel->hide();

            vbox.emplace<Label>().assign(&this->ui_.rumbleMetadataLabel);
            this->ui_.rumbleMetadataLabel->setObjectName(
                QStringLiteral("usercard-rumble-metadata"));
            this->ui_.rumbleMetadataLabel->hide();
    ...
    auto user = layout.emplace<QHBoxLayout>().withoutMargin();
    {
        ...
        auto mod = user.emplace<PixmapButton>(this).assign(&this->ui_.mod);
        mod->setPixmap(getResources().buttons.mod);
        mod->setScaleIndependentSize(30, 30);
        auto unmod = user.emplace<PixmapButton>(this).assign(&this->ui_.unmod);
        unmod->setPixmap(getResources().buttons.unmod);
        unmod->setScaleIndependentSize(30, 30);
        auto vip = user.emplace<PixmapButton>(this).assign(&this->ui_.vip);
        vip->setPixmap(getResources().buttons.vip);
        vip->setScaleIndependentSize(30, 30);
        auto unvip = user.emplace<PixmapButton>(this).assign(&this->ui_.unvip);
    ...
    auto lineMod =
        layout.emplace<Line>(false).assign(&this->ui_.moderationLine);
    ...
    auto moderation = layout.emplace<QHBoxLayout>().withoutMargin();
    {
        ...
        std::ignore = this->userStateChanged_.connect([this, lineMod,
            ...
            else if (userCardSurfacePolicy(this->identity_)
                         .showModeratorActions)
            {
                if (auto *kickChannel = dynamic_cast<KickChannel *>(
    ...
    layout.emplace<Line>(false).assign(&this->ui_.afterModerationLine);
...
void UserInfoPopup::installEvents()
{
    ...
    QObject::connect(
        ...
        [this](int newState) mutable {
            if (this->isKick_ || this->isRumble_)
            {
                return;
            }
                            ...
                            const ChannelPtr &openingChannel)
{
    ...
    const auto type = !contextChannel->isEmpty() ? contextChannel->getType()
                                                 : openingChannel->getType();
    if (type == Channel::Type::Kick)
    {
        platform = MessagePlatform::Kick;
    }
    else if (type == Channel::Type::Rumble)
    {
        platform = MessagePlatform::Rumble;
    }

    auto identity = userCardIdentityFromName(name, platform);
    const QStringView idPrefix = u"id:";
    if (name.startsWith(idPrefix))
    {
        identity.key.kind = UserCardIdentityKind::StableProviderID;
        identity.key.value = name.mid(idPrefix.size());
        identity.loginName.clear();
    }
    this->setData(identity, contextChannel, openingChannel);
}

void UserInfoPopup::setData(const UserCardIdentity &identity,
                            const ChannelPtr &contextChannel,
                            const ChannelPtr &openingChannel)
{
    this->identity_ = identity;
    this->identityGeneration_++;
    ...
    this->userName_ = identity.loginName;
    this->kickUserSlug_ = identity.loginName;
    this->userId_.clear();
    if (identity.key.kind == UserCardIdentityKind::StableProviderID)
    {
        this->userId_ = identity.key.value;
    }
    ...
    const auto titleName = identity.displayName.isEmpty()
                               ? identity.loginName
                               : identity.displayName;
    this->setWindowTitle(
        TEXT_TITLE.arg(titleName, this->underlyingChannel_->getName()));
    this->isKick_ = identity.key.provider == UserCardProvider::Kick;
    this->isRumble_ = identity.key.provider == UserCardProvider::Rumble;
    if (this->isKick_)
    {
        this->ui_.timeoutWidget->setMinTimeout(60);
    }

    this->ui_.nameLabel->setText(titleName);
    this->ui_.nameLabel->setProperty("copy-text", titleName);
    this->ui_.nameLabel->setPalette(QPalette());
    this->applySurfacePolicy();

    switch (userCardProfileLookup(identity))
    {
        case UserCardProfileLookup::Twitch:
            this->updateUserData();
            break;
        case UserCardProfileLookup::Kick:
            this->updateKickUserData();
            break;
        case UserCardProfileLookup::None:
            this->updateRumbleUserData();
            break;
    }

    this->userStateChanged_.invoke();

    if (!identity.loginName.isEmpty())
    {
    ...
    if (type == Channel::Type::TwitchLive ||
        ...
        type == Channel::Type::Kick || this->isRumble_)
    {
...
void UserInfoPopup::applySurfacePolicy()
{
    const auto policy = userCardSurfacePolicy(this->identity_);
    this->ui_.avatarButton->setVisible(policy.showAvatar);
    this->ui_.switchAvatars->setVisible(false);
    this->ui_.followerCountLabel->setVisible(policy.showAccountMetadata);
    this->ui_.createdDateLabel->setVisible(policy.showAccountMetadata);
    this->ui_.followageLabel->setVisible(policy.showAccountMetadata);
    this->ui_.subageLabel->setVisible(policy.showAccountMetadata);
    this->ui_.liveIndicator->setVisible(false);
    this->ui_.userIDLabel->setVisible(true);
    if (this->ui_.pronounsLabel)
    {
        this->ui_.pronounsLabel->setVisible(policy.showAccountMetadata);
    }
    this->ui_.usercardLabel->setVisible(policy.showTwitchProfileAction);
    this->ui_.block->setVisible(policy.showBlock);
    this->ui_.ignoreHighlights->setVisible(policy.showIgnoreHighlights);
    this->ui_.notesAdd->setVisible(policy.showNotes);
    this->ui_.notesPreview->setVisible(false);
    this->ui_.mod->setVisible(false);
    this->ui_.unmod->setVisible(false);
    this->ui_.vip->setVisible(false);
    this->ui_.unvip->setVisible(false);
    this->ui_.timeoutWidget->setVisible(false);
    this->ui_.moderationLine->setVisible(false);
    this->ui_.afterModerationLine->setVisible(policy.showModeratorActions);
    this->ui_.platformBadge->setVisible(this->isRumble_);
    this->ui_.platformLabel->setVisible(this->isRumble_);
    this->ui_.rumbleMetadataLabel->setVisible(false);
    this->ui_.localizedNameLabel->setVisible(false);
    this->ui_.localizedNameCopyButton->setVisible(false);
}

void UserInfoPopup::updateRumbleUserData()
{
    assert(this->isRumble_);

    this->ui_.platformBadge->setPixmap(QPixmap(
        platformBadgeResource(MessagePlatform::Rumble, true).toString()));
    this->ui_.platformLabel->setText(QStringLiteral("Rumble"));

    const auto displayName = this->identity_.displayName.isEmpty()
                                 ? this->identity_.loginName
                                 : this->identity_.displayName;
    this->ui_.nameLabel->setText(displayName);
    this->ui_.nameLabel->setProperty("copy-text", displayName);
    if (this->identity_.usernameColor.isValid())
    {
        auto palette = this->ui_.nameLabel->palette();
        palette.setColor(QPalette::WindowText, this->identity_.usernameColor);
        this->ui_.nameLabel->setPalette(palette);
    }

    if (!this->identity_.loginName.isEmpty() &&
        displayName.compare(this->identity_.loginName, Qt::CaseSensitive) != 0)
    {
        this->ui_.localizedNameLabel->setText(this->identity_.loginName);
        this->ui_.localizedNameLabel->setProperty("copy-text",
                                                  this->identity_.loginName);
        this->ui_.localizedNameLabel->show();
        this->ui_.localizedNameCopyButton->show();
    }

    if (this->identity_.key.kind == UserCardIdentityKind::StableProviderID)
    {
        this->ui_.userIDLabel->setText(TEXT_USER_ID %
                                       this->identity_.key.value);
        this->ui_.userIDLabel->setProperty("copy-text",
                                           this->identity_.key.value);
        this->ui_.userIDLabel->show();
    }
    else
    {
    ...
    const auto roles =
        rumbleUserCardMetadataLabels(this->identity_.rumbleRoleIDs);
    const auto badges =
        rumbleUserCardMetadataLabels(this->identity_.rumbleBadgeIDs);
    if (!roles.isEmpty())
    {
        metadata.append(QStringLiteral("Roles: %1").arg(roles.join(", ")));
    }
    if (!badges.isEmpty())
    {
        metadata.append(QStringLiteral("Badges: %1").arg(badges.join(", ")));
    }
    if (!metadata.isEmpty())
    {
        this->ui_.rumbleMetadataLabel->setText(
            metadata.join(QStringLiteral(" · ")));
        this->ui_.rumbleMetadataLabel->show();
    }
}

bool UserInfoPopup::isCurrentRequest(const UserCardIdentityKey &key,
                                     uint64_t generation) const
{
    return userCardRequestIsCurrent({key, generation}, this->identity_.key,
                                    this->identityGeneration_);
}
...
void UserInfoPopup::updateLatestMessages()
{
    auto filteredChannel =
        filterMessages(this->identity_, this->underlyingChannel_);
        ...
        std::make_unique<pajlada::Signals::ScopedConnection>(
            this->underlyingChannel_->messageAppended.connect(
                [this, hasMessages](auto message, auto) {
                    if (!messageMatchesUserCard(this->identity_, *message))
                    {
                        return;
                    }
...
void UserInfoPopup::updateUserData()
{
    ...
    const auto requestKey = this->identity_.key;
    const auto requestGeneration = this->identityGeneration_;
    const auto requestIsCurrent = [this, hack, requestKey, requestGeneration] {
        return hack.lock() &&
               this->isCurrentRequest(requestKey, requestGeneration);
    };
    auto currentUser = getApp()->getAccounts()->twitch.getCurrent();

    const auto onUserFetchFailed = [this, requestIsCurrent] {
        if (!requestIsCurrent())
        {
            return;
        }
    ...
    const auto onUserFetched = [this, requestIsCurrent, requestKey,
                                requestGeneration,
        ...
        {
            this->loadAvatar(user.id, user.profileImageUrl, false, requestKey,
                             requestGeneration);
        }

        getHelix()->getChannelFollowers(
            user.id,
            [this, requestIsCurrent](const auto &followers) {
                if (!requestIsCurrent())
                {
                    return;
                }
        ...
        getHelix()->getStreamById(
            user.id,
            [this, requestIsCurrent](bool isLive, const auto &stream) {
                if (!requestIsCurrent())
                {
                    return;
                }
        ...
        if (type == Channel::Type::Twitch)
        {
            ...
            getIvr()->getSubage(
                ...
                [this, requestIsCurrent](const IvrSubage &subageInfo) {
                    if (!requestIsCurrent())
                    {
                        return;
                    }
        ...
        if (getSettings()->showPronouns)
        {
            getApp()->getPronouns()->getUserPronoun(
                user.login,
                [this, requestIsCurrent](const auto userPronoun) {
                    runInGuiThread([this, requestIsCurrent,
...
void UserInfoPopup::loadAvatar(const QString &userID, const QString &pictureURL,
                               bool isKick, const UserCardIdentityKey &key,
                               uint64_t generation)
{
    if (!this->isCurrentRequest(key, generation))
    {
        return;
    }
    auto filename =
    ...
    else
    {
        ...
        QObject::connect(reply, &QNetworkReply::finished, this,
                         [this, reply, filename, key, generation] {
                             if (!this->isCurrentRequest(key, generation))
                             {
                                 return;
                             }
                                      ...
                                      uint64_t generation)
{
    ...
    NetworkRequest(fmt.arg(userID))
        ...
        .onSuccess([this, hack = std::weak_ptr<bool>(this->lifetimeHack_), key,
            ...
            if (cacheFile.exists())
            {
                this->setSevenTVAvatar(filename, format, key, generation);
                return;
            }
            ...
            QObject::connect(
                ...
                [this, reply, url, filename, format, key, generation] {
                    if (!this->isCurrentRequest(key, generation))
                    {
                        return;
                    }
                    if (reply->error() == QNetworkReply::NoError)
                    {
                        ...
                        this->setSevenTVAvatar(filename, format, key,
                                               generation);
                    }
                    else
                    {
                                     ...
                                     uint64_t generation)
{
    ...
    QObject::connect(
        movie, &QMovie::frameChanged, this, [this, movie, key, generation] {
            if (this->isCurrentRequest(key, generation))
            {
...
void UserInfoPopup::updateKickUserData()
{
    ...
    auto onChannelFetchFailed = [requestKey,
                                 requestGeneration](UserInfoPopup *self) {
        if (!self->isCurrentRequest(requestKey, requestGeneration))
        {
            return;
        }
        // this can occur when the account doesn't exist.
    ...
    auto onChannelFetched = [requestKey, requestGeneration](
                                UserInfoPopup *self,
        ...
        {
            self->loadAvatar(userIDStr, self->helixAvatarUrl_, true, requestKey,
                             requestGeneration);
        }
    ...
    KickApi::privateUserInChannelInfo(
        this->userName_, this->underlyingChannel_->getName(),
        [self = QPointer(this), requestKey,
         requestGeneration](const auto &res) {
            if (!self || !res ||
                !self->isCurrentRequest(requestKey, requestGeneration))
            {
                return;
            }
...
{
    if (!userCardSurfacePolicy(this->identity_).showCommonProfileActions)
    {
        return;
    }
