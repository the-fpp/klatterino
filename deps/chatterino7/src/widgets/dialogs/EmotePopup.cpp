...
class AllEmoteItemsStrategy final
    : public completion::EmoteSource::EmoteStrategy
{
public:
...
completion::EmoteCandidateResolution resolvePickerCandidate(
    const ChannelPtr &displayChannel, const ChannelPtr &provenanceChannel,
    const EmotePtr &emote, QStringView insertionText)
{
    completion::EmoteSource displayResolver(displayChannel.get(), nullptr);
    auto displayed = displayResolver.resolveCandidate(emote, insertionText);
    ...
    const auto *multi =
        dynamic_cast<const MultiChannel *>(provenanceChannel.get());
    if (multi == nullptr)
    {
        return displayed;
    }

    const bool displayIsCurrentChild =
        std::ranges::any_of(multi->channels(), [&](const auto &child) {
            return child.channel == displayChannel;
        });
    if (!displayIsCurrentChild)
    {
        return {.candidate = std::nullopt, .ambiguous = true};
    }

    std::optional<DraftEmoteCandidate> aggregate;
    for (const auto &child : multi->channels())
    {
        // Picker clicks are infrequent. Enumerating each child's in-memory
        // source here lets us match by stable identity rather than EmotePtr;
        // aggregate deduplication intentionally keeps only one child's pointer.
        completion::EmoteSource childSource(
            child.channel.get(), std::make_unique<AllEmoteItemsStrategy>());
        childSource.update(QString{});
        for (const auto &item : childSource.output())
        {
            if (item.identity != displayed.candidate->identity ||
                item.tabCompletionName != insertionText)
            {
                continue;
            }

            const auto candidate =
                completion::draftCandidateFromEmoteItem(item);
            if (!candidate)
            {
                return {.candidate = std::nullopt, .ambiguous = true};
            }
            ...
            {
                appendUniqueAvailability(
                    aggregate->availabilityAlternatives, availability);
            }
        }
    }
    ...
    return {.candidate = std::move(aggregate), .ambiguous = false};
}
    ...
    , notebook_(new Notebook(this))
{
    ...
    auto clicked = [this](const MessageLayoutElement *hoveredElement,
        ...
        {
            const auto *element = dynamic_cast<const EmoteElement *>(
                &hoveredElement->getCreator());
            if (element && this->channel_ && this->provenanceChannel_)
            {
                auto resolution = resolvePickerCandidate(
                    this->channel_, this->provenanceChannel_,
                    element->getEmote(), link.value);
                candidate = std::move(resolution.candidate);
                provenanceAmbiguous = resolution.ambiguous;
            }
            else
            {
        ...
        this->linkClicked.invoke(link, std::move(candidate),
                                 provenanceAmbiguous);
    };
...
void EmotePopup::loadChannel(ChannelPtr displayChannel,
                             ChannelPtr provenanceChannel)
{
    BenchmarkGuard guard("loadChannel");

    this->channel_ = std::move(displayChannel);
    this->provenanceChannel_ = provenanceChannel
                                   ? std::move(provenanceChannel)
                                   : this->channel_;
