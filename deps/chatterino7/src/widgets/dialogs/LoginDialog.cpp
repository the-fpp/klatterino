...
TwitchLoginWidget::TwitchLoginWidget()
{
    this->setLayout(&this->layout_);
    this->layout_.addWidget(&this->basic_);

    this->advancedToggle_.setText(QStringLiteral("Advanced Twitch login"));
    this->advancedToggle_.setCheckable(true);
    this->advancedToggle_.setArrowType(Qt::RightArrow);
    this->advancedToggle_.setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
    this->layout_.addWidget(&this->advancedToggle_);

    this->advanced_.hide();
    this->layout_.addWidget(&this->advanced_);
    QObject::connect(&this->advancedToggle_, &QToolButton::toggled, this,
                     [this](bool expanded) {
                         this->advancedToggle_.setArrowType(
                             expanded ? Qt::DownArrow : Qt::RightArrow);
                         this->advanced_.setVisible(expanded);
                         if (auto *dialog = this->window())
                         {
                             dialog->adjustSize();
                         }
                     });
...
RumbleLoginPage::RumbleLoginPage()
{
    this->setLayout(&this->layout_);
    this->description_.setWordWrap(true);
    this->description_.setText(QStringLiteral(
        "Sign in to Rumble in a separate window. It closes when sign-in is "
        "complete. Chatterino saves your account using your system's secure "
        "storage."));
    this->layout_.addWidget(&this->description_);

    this->status_.setWordWrap(true);
    this->layout_.addWidget(&this->status_);

    this->progress_.setRange(0, 0);
    this->progress_.hide();
    this->layout_.addWidget(&this->progress_);

    this->loginButton_.setText(QStringLiteral("Sign in to Rumble"));
    this->loginButton_.setEnabled(false);
    this->layout_.addWidget(&this->loginButton_);
    this->layout_.addStretch();

    QObject::connect(&this->loginButton_, &QPushButton::clicked, this, [this] {
        auto *app = getApp();
        if (app->getRumble()->browserLoginActive())
        {
            app->getRumble()->cancelBrowserLogin();
            this->refresh();
            return;
        }
        const auto status = app->getRumble()->browserLoginStatus();
        if (status.readiness != rumble::BrowserLoginReadiness::Ready)
        {
            this->preparationStarted_ = false;
            this->prepare();
            return;
        }
        this->status_.setText(
            QStringLiteral("Complete sign-in in the %1 window. It will close "
                           "automatically.")
                .arg(status.browserName));
        this->progress_.setRange(0, 0);
        this->progress_.show();
        this->loginButton_.setText(QStringLiteral("Cancel Rumble sign-in"));
        app->getRumble()->loginInBrowser([guarded = QPointer<RumbleLoginPage>(
                                              this)](bool ok, QString message) {
            if (!guarded)
                return;
            guarded->status_.setText(message);
            guarded->progress_.hide();
            guarded->loginButton_.setText(QStringLiteral("Sign in to Rumble"));
            if (ok && guarded->window())
            {
                guarded->window()->close();
            }
            else
            {
                guarded->loginButton_.setEnabled(true);
            }
        });
    });
}

void RumbleLoginPage::prepare()
{
    if (this->preparationStarted_)
    {
        return;
    }
    this->preparationStarted_ = true;
    if (!getApp()->getAccounts()->rumble.secureStorageAvailable())
    {
        this->status_.setText(QStringLiteral(
            "Secure credential storage is unavailable. Unlock or start "
            "your system keyring, then try again."));
        this->progress_.hide();
        this->loginButton_.setText(
            QStringLiteral("Retry secure storage check"));
        this->loginButton_.setEnabled(true);
        return;
    }

    this->status_.setText(
        QStringLiteral("Checking browser support for Rumble sign-in…"));
    this->progress_.setRange(0, 0);
    this->progress_.show();
    this->loginButton_.setEnabled(false);
    getApp()->getRumble()->prepareBrowserLogin(
        [guarded =
             QPointer<RumbleLoginPage>(this)](rumble::BrowserLoginStatus) {
            if (!guarded)
                return;
            guarded->refresh();
        });
...
void RumbleLoginPage::refresh()
{
    auto *rumbleController = getApp()->getRumble();
    if (rumbleController->browserLoginActive())
    {
        this->progress_.setRange(0, 0);
        this->progress_.show();
        this->loginButton_.setEnabled(true);
    ...
    const auto status = rumbleController->browserLoginStatus();
    this->status_.setText(status.userMessage);
    this->progress_.setVisible(status.readiness ==
                               rumble::BrowserLoginReadiness::Preparing);
    this->loginButton_.setText(
        status.readiness == rumble::BrowserLoginReadiness::Ready
            ? QStringLiteral("Sign in to Rumble")
            : QStringLiteral("Retry sign-in setup"));
    this->loginButton_.setEnabled(
        status.readiness == rumble::BrowserLoginReadiness::Ready ||
        status.readiness == rumble::BrowserLoginReadiness::Unavailable ||
        status.readiness == rumble::BrowserLoginReadiness::Failed);
}
    ...
    : QDialog(parent)
{
    ...
    this->ui_.tabWidget.addTab(&this->ui_.twitch, "Twitch");
    this->ui_.tabWidget.addTab(&this->ui_.kick, "Kick");
    this->ui_.tabWidget.addTab(&this->ui_.rumble, "Rumble");
    ...
    QObject::connect(
        &this->ui_.tabWidget, &QTabWidget::currentChanged, this,
        [this](int index) {
            if (this->ui_.tabWidget.widget(index) == &this->ui_.rumble)
            {
                this->ui_.rumble.prepare();
            }
        });
}
