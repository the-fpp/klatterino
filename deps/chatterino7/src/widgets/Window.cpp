...
#ifdef CHATTERINO_HAVE_PLUGINS
    , pluginControlTabHost_(
          type == WindowType::Main || type == WindowType::Popup
              ? new PluginControlTabHost(this)
              : nullptr)
#endif
{
    ...
    QObject::connect(this, &Window::compactModeChanged, this,
                     [this](bool compact) {
                         this->notebook_->setCompactMode(compact);
                         this->updateCompactTabControls();
                     });
    QObject::connect(this->notebook_, &Notebook::compactTabStatusChanged, this,
                     &Window::updateCompactTabControls);
    QObject::connect(this->notebook_, &Notebook::compactTabPresentationChanged,
                     this, &Window::updateCompactTabControls);
    this->updateCompactTabControls();
...
#ifdef CHATTERINO_HAVE_PLUGINS
PluginControlTabHost *Window::getPluginControlTabHost() const
{
    return this->pluginControlTabHost_;
}
#endif

bool Window::event(QEvent *event)
{
    const bool updateCompactMode =
    ...
    const auto handled = BaseWindow::event(event);
    if (updateCompactMode)
    {
        this->connectCompactModeWindow();
        this->updateCompactMode();
    }
    return handled;
}

void Window::connectCompactModeWindow()
{
    auto *handle = this->windowHandle();
    if (this->compactModeWindowHandle_ == handle)
    {
        return;
    }

    QObject::disconnect(this->compactModeWindowScreenConnection_);
    this->compactModeWindowHandle_ = handle;
    if (handle == nullptr)
    {
        this->compactModeWindowScreenConnection_ = {};
        return;
    }

    this->compactModeWindowScreenConnection_ = QObject::connect(
        handle, &QWindow::screenChanged, this, [this](QScreen *screen) {
            this->connectCompactModeScreen(screen);
            this->updateCompactMode();
        });
}

void Window::connectCompactModeScreen(QScreen *screen)
{
    if (this->compactModeScreen_ == screen)
    {
        return;
    }

    for (const auto &connection : this->compactModeScreenConnections_)
    {
        QObject::disconnect(connection);
    }
    this->compactModeScreenConnections_.clear();
    this->compactModeScreen_ = screen;
    if (screen == nullptr)
    {
        return;
    }
    ...
    this->compactModeScreenConnections_.push_back(QObject::connect(
        screen, &QScreen::availableGeometryChanged, this, update));
    this->compactModeScreenConnections_.push_back(
        QObject::connect(screen, &QScreen::geometryChanged, this, update));
    this->compactModeScreenConnections_.push_back(QObject::connect(
        screen, &QScreen::logicalDotsPerInchChanged, this, update));
}

QScreen *Window::containingScreen() const
{
    auto *screen = QGuiApplication::screenAt(this->frameGeometry().center());
...
void Window::updateCompactMode()
{
    auto *screen = this->containingScreen();
    this->connectCompactModeScreen(screen);

    const ResponsiveTabGeometry geometry{
        .windowHeight = this->height(),
        .availableScreenHeight =
            screen == nullptr ? 0 : screen->availableGeometry().height(),
        .maximized = this->isMaximized(),
        .fullscreen = this->isFullScreen(),
    };
    const bool compact = shouldUseCompactTabs(geometry, this->compactMode_);
    if (compact == this->compactMode_)
    {
        return;
    }

    this->compactMode_ = compact;
    Q_EMIT this->compactModeChanged(compact);
}
...
#ifdef CHATTERINO_HAVE_PLUGINS
    if (this->pluginControlTabHost_ != nullptr)
    {
        layout->addWidget(this->pluginControlTabHost_);
    }
#endif
...
void Window::addCustomTitlebarButtons()
{
    ...
    // addTitleBarLabel inserts at the front. Create in reverse visual order so
    // the group is previous-live, previous, status, next, next-live immediately
    // beside the account label when one is present. Popup/attached windows need
    // navigation too.
    this->compactTabControls_.nextLive = this->addTitleBarLabel([this] {
        this->notebook_->selectNextLiveTab();
    });
    this->compactTabControls_.next = this->addTitleBarLabel([this] {
        this->notebook_->selectNextTab();
    });
    this->compactTabControls_.status = this->addTitleBarLabel([] {});
    this->compactTabControls_.previous = this->addTitleBarLabel([this] {
        this->notebook_->selectPreviousTab();
    });
    this->compactTabControls_.previousLive = this->addTitleBarLabel([this] {
        this->notebook_->selectPreviousLiveTab();
    });
    ...
    QObject::connect(this->compactTabControls_.status,
                     &LabelButton::leftClicked, this, [this] {
                         this->notebook_->setCompactTabsRevealed(true);
                     });
    // This titlebar group is layout-managed, unlike Notebook's manually
    // positioned controls whose arrow widths follow the tab height.
    this->compactTabControls_.previousLive->setScaleIndependentWidth(30);
    this->compactTabControls_.previous->setScaleIndependentWidth(30);
    this->compactTabControls_.next->setScaleIndependentWidth(30);
    this->compactTabControls_.nextLive->setScaleIndependentWidth(30);
...
void Window::updateCompactTabControls()
{
    applyCompactTabControlState(
        this->compactTabControls_, this->notebook_->getCompactTabStatus(),
        this->notebook_->getCompactSelectedTitle(),
        this->compactMode_ && !this->notebook_->areCompactTabsRevealed());
}
