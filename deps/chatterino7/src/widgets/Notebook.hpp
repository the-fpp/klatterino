...
public:
     ...
     * Selects the first visible tab when no page is selected.
     **/
    void selectNextTab(bool focusPage = true);

    /**
     * @brief Selects the previous visible tab. Wraps to the end if required.
     * Selects the last visible tab when no page is selected.
     **/
    void selectPreviousTab(bool focusPage = true);

    /// Selects the next visible live tab in notebook order with wraparound.
    /// Does nothing unless at least two visible live destinations exist.
    void selectNextLiveTab(bool focusPage = true);

    /// Selects the previous visible live tab in notebook order with
    /// wraparound. Does nothing unless at least two visible live destinations
    /// exist.
    void selectPreviousLiveTab(bool focusPage = true);
    ...
    /// Responsive compact mode only changes presentation. It does not mutate
    /// the user's tab-visibility setting, filter, order, or selection.
    bool isCompactMode() const;
    void setCompactMode(bool compact);
    bool areCompactTabsRevealed() const;
    void setCompactTabsRevealed(bool revealed);

    CompactTabStatus getCompactTabStatus() const;
    const QString &getCompactSelectedTitle() const;
...
Q_SIGNALS:
    /// Emitted when the position, normal/live count, enablement, or selected
    /// title used by compact controls changes.
    void compactTabStatusChanged();
    /// Emitted when compact controls and temporarily revealed tabs exchange
    /// places without changing the responsive compact-mode state.
    void compactTabPresentationChanged();
...
protected:
    ...
    /// Adds compact controls to the notebook-hosted profile/control strip.
    void addCompactTabControls();
...
private:
    ...
    bool shouldShowNormalTabs() const;
    int customButtonWidth(const Button *button, const LayoutContext &ctx) const;
    void updateCompactTabStatus();
    void updateCompactTabControls();
    void selectAdjacentLiveTab(bool forward, bool focusPage);
    ...
    CompactTabStatus compactTabStatus_;
    QString compactSelectedTitle_;

    bool allowUserTabManagement_ = false;
    bool showTabs_ = true;
    bool showAddButton_ = false;
    bool compactMode_ = false;
    bool compactTabsRevealed_ = false;
    int lineOffset_ = 20;
