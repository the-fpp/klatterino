                                     ...
                                     InferChannel mode) const
{
    ...
    auto matches = [&](const MultiChannel::ChildChannel &chan) {
        return multiChannelChildMatches(chan, msg.platform, nameView);
    };
...
void ChannelView::setChannel(const ChannelPtr &underlyingChannel)
{
    ...
    auto handleChan = [this](Channel *chan) {
        ...
        else if (auto *rumbleChannel = dynamic_cast<RumbleChannel *>(chan))
        {
            this->channelConnections_.managedConnect(
                rumbleChannel->liveStatusChanged, [this] {
                    this->liveStatusChanged.invoke();
                });
        }
    };
                                    ...
                                    const QString &alternativePopoutChannel)
{
    ...
    if (!openingChannel)
    {
        openingChannel = Channel::getEmpty();
    }
    ...
    else if (platform == MessagePlatform::Rumble)
    {
        // Rumble user popout/context ownership lands with the channel work.
        contextChannel = Channel::getEmpty();
    }
    else
    {
                                    ...
                                    const Message &sourceMessage)
{
    ...
    const auto identity = userCardIdentityFromMessage(sourceMessage, userName);
    auto openingChannel = this->effectiveSourceChannel();

    ChannelPtr base =
        this->sourceChannel_ ? this->sourceChannel_ : this->underlyingChannel_;
    if (auto *multi = dynamic_cast<MultiChannel *>(base.get()))
    {
        const auto findMatchingChild = [&](QStringView sourceName) {
            return std::ranges::find_if(
                multi->channels(), [&](const auto &child) {
                    return multiChannelChildMatches(
                        child, sourceMessage.platform, sourceName);
                });
        };

        auto found = findMatchingChild(sourceMessage.channelName);
        if (found == multi->channels().end() && sourceMessage.rumble)
        {
            found = findMatchingChild(sourceMessage.rumble->channelID);
        }
        if (found != multi->channels().end())
        {
            openingChannel = found->channel;
        }
    }

    ChannelPtr contextChannel;
    if (identity.key.provider == UserCardProvider::Kick)
    {
        contextChannel = getApp()->getKickChatServer()->findBySlug(
            sourceMessage.channelName);
    }
    else if (identity.key.provider == UserCardProvider::Rumble)
    {
        if (openingChannel && openingChannel->isRumbleChannel())
        {
    ...
    {
        contextChannel =
            getApp()->getTwitch()->getChannelOrEmpty(sourceMessage.channelName);
    }
    if (!contextChannel)
    {
        contextChannel = Channel::getEmpty();
    ...
    auto *userPopup =
        new UserInfoPopup(getSettings()->autoCloseUserPopup, this->split_);
    userPopup->setData(identity, contextChannel, openingChannel);
                                  ...
                                  MessageLayout *layout)
{
    ...
    switch (link.type)
    {
        ...
        case Link::UserInfo: {
            ...
            this->showUserInfoPopup(user, *layout->getMessage());
        }
        break;
        ...
        case Link::Reconnect: {
            if (this->split_)
            {
                this->split_->reconnect();
            }
            else
            {
                this->underlyingChannel_->reconnect();
            }
        }
        break;
